...
namespace Invector.vCharacterController
{
    public class vThirdPersonMotor : vCharacter
    {
        ...
        [vEditorToolbar("Slide", order = 4)]
        public bool useSlideRootMotion = true;
        [Tooltip("Can control the Slide Direction")]
        public bool slideControl = true;
        [Tooltip("Speed of the Slide Movement")]
        public float slideSpeed = 0f;
        [Tooltip("Speed of the Slide Rotation")]
        public float slideRotationSpeed = 10f;
        [vHideInInspector("Slide use gravity inflence")]
        public bool slideUseGravity = true;
        [vHideInInspector("slideUseGravity")]
        [Tooltip("Normalized Time of the slide animation to enable gravity influence")]
        public float slideUseGravityTime = 0.2f;
        [Tooltip("Use the normalized time of the animation to know when you can slide again")]
        [Range(0, 1)]
        public float timeToSlideAgain = 0.75f;
      ...
      //  [Tooltip("Delay to start sliding once the character is standing on a slope")]
       ...
       // internal float _slidingEnterTime;
        ...
        public virtual void DeathBehaviour()
        {
            ...
            vThirdPersonAnimator.triggerDieBehaviour = false;
        ...
        public virtual void MoveCharacter(Vector3 _direction)
        {
            ...
            if (ragdolled || !isGrounded || isJumping) return;
        ...
        private void SnapToGround(ref Vector3 targetVelocity, ref bool useVerticalVelocity)
        {
            ...
            var snapConditions = isGrounded && groundHit.collider != null && GroundAngle() <= slopeLimit && !disableCheckGround && !isJumping && !customAction && input.magnitude > 0.1f && !isInAirborne;
            if (snapConditions)
            {
        ...
        {
            if (isGrounded && !disableCheckGround && !isJumping && !customAction && !isInAirborne)
            {
        ...
        {
            if (lockAnimRotation || customAction || (!jumpAndRotate && !isGrounded) || ragdolled) return;
            direction.y = 0f;
        ...
        public virtual void AirControl()
        {
            ...
            if (jumpWithRigidbodyForce && !isGrounded || jumpWithRigidbodyForce && !isGrounded|| isDead)
            {
        ...
        public virtual void ControlCapsuleHeight()
        {
            ...
            {
                _capsuleCollider.center = colliderCenter / 2;// crouchHeightReduction;
                _capsuleCollider.height = colliderHeight / 2;// crouchHeightReduction;
            }
            else
            {
